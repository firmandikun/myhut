...
@section('content')
    <div class="dashboard-main-body">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
            <h6 class="fw-semibold mb-0">Edit Transaksi Pembelian</h6>
        </div>

        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Edit Transaksi Pembelian</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('purchases.update', $purchase->id) }}" method="POST">
                        ...
                        <select class="form-select" id="product_id" name="product_id" required>
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" data-price="{{ $product->price }}"
                                    data-stock="{{ $product->stock }}" {{ $purchase->product_id == $product->id ? 'selected' : '' }}>
                                    {{ $product->name }} (Harga Beli: Rp {{ number_format($product->price, 0, ',', '.') }})
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" id="price_display" readonly value="Rp {{ number_format($purchase->price_per_unit, 0, ',', '.') }}">
                            <input type="hidden" id="price_per_unit" name="price_per_unit" value="{{ $purchase->price_per_unit }}">
                        </div>
                    </div>

                    <div class="mb-3">
                        <label for="purchase_date" class="form-label">Tanggal Pembelian</label>
                        <input type="date" class="form-control" id="purchase_date" name="purchase_date" value="{{ $purchase->purchase_date->format('Y-m-d') }}" required>
                    </div>
